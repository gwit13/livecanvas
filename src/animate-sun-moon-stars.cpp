...
// Only sky bodies + stars are driven; keep the USB cap.
...
// Fixed panel-space star field (does not twinkle or move).
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("animate-sun-moon-stars: sun then moon+stars");
  Serial.printf("  origin=(%d,%d) inset L=%d R=%d\n",
                PAINTING_ORIGIN_X, PAINTING_ORIGIN_Y,
                SET_INSET_LEFT, SET_INSET_RIGHT);
  Serial.printf("  MASK_FG_CLOUDS=%d MASK_BG_CLOUDS=%d MASK_HORIZON=%d\n",
                MASK_FOREGROUND_CLOUDS, MASK_BACKGROUND_CLOUDS, MASK_HORIZON);
  Serial.printf("  moon_r=%.2f stars=%d\n", MOON_RADIUS, STAR_COUNT);
