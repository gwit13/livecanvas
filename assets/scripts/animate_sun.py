#!/usr/bin/env python3
"""Load painting_rgb.h as a numpy grid and animate a sun across the sky.

The sun has a floating-point position on the 36x28 logical grid (origin
top-left, x right, y down). A render tick samples that position onto the
discrete cells as a radius-1 disk (about 2x2 when it sits between cells).

Path is a hemisphere in 10s. Sunset is 3 panel LEDs in from the right
edge; the apex drops with the shorter arc. Preview is the 32x24 panel
window into the painting. Pan with arrow keys or --origin-x/--origin-y.
"""
...
def splat_sun(
    ...
    """Composite a radius-1 disk at a sub-pixel (row, col).

    Coverage is 1 at the sun center and falls off to 0 at `radius`, with a
    short antialias so a position sitting between two cells lights both.
    """
    ...
    # Disk of radius 1: one cell when parked on a grid point, two cells
    # when halfway between neighbors, 2x2 on a cell corner. A short
    # antialias ramp keeps the edge from popping on/off in one tick.
    aa = 0.25
...
def render_tick(background: np.ndarray, t: float) -> np.ndarray:
    row, col = sun_position(t)
    return splat_sun(background, row, col)
...
def run_window(
    ...
    def tick() -> None:
        ...
        preview = upscale(crop_to_panel(render_tick(background, t), align), cell)
        if state["collect"]:
...
def main() -> None:
    ...
    background = load_painting(args.header)
    ...
    if args.no_window:
        ...
        frames = [
            upscale(
                crop_to_panel(render_tick(background, i / (n_frames - 1)), align),
                args.cell,
            )
